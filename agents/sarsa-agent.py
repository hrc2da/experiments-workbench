    ...
    def run(self, environment, n_steps, logger=None, exc_logger=None, status=None, initial=None, eps=0.8, eps_decay=0.9,
        ...
        while i < n_steps:
            ...
            for j in range(n_tries_per_step):
                ...
                rewards = []
                samples += 1
                neighborhood = environment.get_neighbors(1,0) #only moving first block of first district
                if len(neighborhood) < 1:
                    continue
                else:
                    metrics = [environment.get_metrics(n, exc_logger) for n,s in neighborhood]
                    count += len(metrics)
