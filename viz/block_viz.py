    ...
    for x in j_file[0]["episodes"]:
        for y in x["run_log"]:
            for keys, val in y["design"].items():
                ...
                plt.plot([int(coordx)], [int(coordy)], 'ro')
                print(final_coords)
    plt.savefig(os.path.join(os.getcwd(),"{}.png".format("agent_sarsa_episodes_100_episodelen_100_subsample_50_alpha_0.9_disc_0.2_optimistic_[-1.0.0.]")))
